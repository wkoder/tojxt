...
class Persistence():
    ...
    def _existsId(self, table, value, columnId = "id"):
    ...
    def isSolvedByUser(self, userId, problemId):
        self.cursor.execute("SELECT COUNT(*) FROM runs WHERE status = 'Accepted' AND userId = ? AND problemId = ?", [userId, problemId])
        data = self.cursor.fetchone()
        return data[0] > 0
    ...
    def getRanking(self, top, country=None):
        where = "" if country is None else "WHERE country LIKE '%s'" % country
        query = "SELECT %s FROM users %s ORDER BY solved DESC, ratio DESC, submitted LIMIT %d" % (USER_COLUMNS, where, top)
